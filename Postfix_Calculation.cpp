...
int Operating(int _left, int _right, char _op)
{
	switch (_op)
	{
	case '*':
...
void PostfixCalculation(const string& pp)
{
	Stack<int> stack;
	stack.Init();
	int Result = 0;
	...
	for (int i = 0; i < Length; ++i)
	{
		...
		else if (isOperator(pp[i]))
		{
			...
			else
			{
				...
				stack.Pop();
				op = pp[i];
				tempResult = Operating(left, right, op);
